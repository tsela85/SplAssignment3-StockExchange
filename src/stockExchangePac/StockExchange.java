...
public class StockExchange implements Listener {
	...
	private Map<String, Company> initCompanies() throws FileNotFoundException,
			IOException {
	...
	private void connectNewClients() {
		...
		for (String client : _newClients) {
			...
			if (broker.getNumOfClients() == N) {
				_stockExchangeStompClient.send("/topic/Connected",
						"connectFailed " + client + "\n");
				_numActiveClients++;
			} else {
				if (broker.getNumOfClients() == 0)
					_numActiveCBrokers++;
				broker.addClient(client);
				_stockExchangeStompClient.send("/topic/cConnected",
						"connected " + client + " " + broker.getName() + "\n");
				// _stockExchangeStompClient.subscribe("/topic/cDeals-"+client,
				// this);
				_numActiveClients++;
			}
			_brokers.add(broker);
