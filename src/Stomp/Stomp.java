...
public abstract class Stomp {
	 ...
	 * confirmed that the commit was successfull.
	 */
	...
	private String addReceipt(Map header) {
		if (header == null)
			header = new HashMap();
		String receipt = String.valueOf(hashCode()) + "&"
	 ...
	 *            The name of the channel to unsubscribe from.
	 */
	public void unsubscribeW(String name, Map header)
			throws InterruptedException {
		String receipt = addReceipt(header);
		unsubscribe(name, (HashMap) null);
		waitOnReceipt(receipt);
	...
	public String nextError() {
		 ...
		 * StringBuffer buff = new StringBuffer(); for (int i=0; i<trace.length;
		 * buff.append(
		 * trace[i].getClassName()+":"+trace[i++].getLineNumber()+" << " ));
		 * System
		 * .err.println("In nextError with "+_errors.size()+" errors from "
		 * +buff.toString());
		 */
